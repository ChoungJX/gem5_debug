...
processor = SwitchableProcessor(
    starting_cores="boot",
    switchable_cores={
        "boot":[SimpleCore(cpu_type=CPUTypes.TIMING, core_id=0, isa=ISA.RISCV),SimpleCore(cpu_type=CPUTypes.TIMING, core_id=1, isa=ISA.RISCV)],
...
class SkylakeProcessor(BaseCPUProcessor):
    ...
    def __init__(self):
        ...
        skylakecore=[TunedCore(cpu_type=CPUTypes.TIMING, core_id=0,), TunedCore(cpu_type=CPUTypes.TIMING, core_id=1,)]

        super().__init__(
            cores = skylakecore
        )

from board.riscv_board import RiscvBoard_cust
board = RiscvBoard_cust(
    clk_freq="3GHz",
    processor=processor,
    memory=memory,
    # cache_hierarchy=ThreeLevelCacheHierarchy(),
    cache_hierarchy=test_cache,
    dist_id=parser.parse_args().number
)
...
if parser.parse_args().number == 0:
    board.set_kernel_disk_workload(    
        kernel = KernelResource("/home/linfeng/bin/riscv/riscv-bootloader-vmlinux-5.10"),
        disk_image = DiskImageResource("/home/linfeng/work/gem5-resources/src/riscv-ubuntu/disk-image/ubuntu_new.img",
                                       root_partition="1"),
    )
else:
    board.set_kernel_disk_workload(    
